...
@WebMvcTest(controllers = HelloController.class,  // 여러 스프링 테스트 어노테이션중, Web(Spring MVC)에 집중할 수 있는 어노테이션이다. (@Service, @Component, @Repository 등은 사용할 수 없다.)
    excludeFilters = {
...
public class HelloControllerTest {
    ...
    public void hello가_리턴된다() throws Exception {
        ...
        mvc.perform(get("/hello"))  // MockMvc를 통해 /hello 주소로 HTTP GET 요청을 한다. 체이닝이 지원되어 아래와 같이 여러 검증 기능을 이어서 선언할 수 있다.
                .andExpect(status().isOk())  // mvc.perform의 결과를 검증한다. HTTP Header의 Status를 검증한다. (200, 404, 500 등의 상태를 검증, 여기선 OK 즉, 200인지 아닌지 검증)
